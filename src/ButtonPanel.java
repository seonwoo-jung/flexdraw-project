...
class ButtonPanel extends JPanel {
    public ButtonPanel(AppState state, History history, DrawingCanvas canvas, ShapeStore store, Runnable refreshProps) {
        ...
        // Tool buttons stored in a HashMap
        ...
        undoBtn.addActionListener(e -> { history.undo(); canvas.repaint(); refreshProps.run(); });
        redoBtn.addActionListener(e -> { history.redo(); canvas.repaint(); refreshProps.run(); });
        ...
        add(selectBtn); add(rectBtn); add(ellBtn); add(lineBtn);
        add(undoBtn); add(redoBtn); add(delBtn); add(frontBtn); add(backBtn);
    }
}
