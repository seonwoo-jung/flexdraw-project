...
class PropertyEditorPanel extends JPanel {
            ...
            for (Map.Entry<String, Object> e : boundShape.getProps().entrySet()) {
                ...
                if (v instanceof Color c) {
                    ...
                    colorBtn.addActionListener(ae -> {
                        ...
                        if (chosen != null) colorBtn.setText(colorToHex(chosen));
                    });
                    input = colorBtn;
                } else {
    ...
    private static Color parseColor(String s) {
        try {
            ...
            if (s.startsWith("0x") || s.startsWith("0X")) return Color.decode(s);
            // allow "r,g,b"
            if (s.contains(",")) {
                String[] p = s.split(",");
                return new Color(
    ...
    private void applyChanges() {
        ...
        for (FieldRow r : rows) {
            ...
            if (r.input instanceof JButton b && r.type == Color.class) {
                ...
                try { parsed = Integer.parseInt(((JTextField) r.input).getText().trim()); }
                catch (Exception ex) { parsed = 0; }
            } else if (r.type == Double.class || r.type == Float.class || r.type == Long.class) {
                try { parsed = Double.parseDouble(((JTextField) r.input).getText().trim()); }
                catch (Exception ex) { parsed = 0.0; }
